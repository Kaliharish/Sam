...
TELEGRAM_BOT_TOKEN = '8163085924:AAG1kBlcmHguYGjsBDWNriTN_dc4tWN-shY'
ADMIN_USER_ID = 7529011141
...
async def manage(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ You need admin approval to use this command. DM ADMIN FOR APPROVAL - @INFLICT_BGMI*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ You need to be approved to use this bot. DM ADMIN FOR APPROVAL - @INFLICT_BGMI*", parse_mode='Markdown')
        return
